...
import {
  acceptWarningSignSuggestion,
  rejectWarningSuggestion
} from 'slices/suggestion/safety/warning/action';
...
export const SuggestedItem: React.FC<Props> = ({ item }) => {
  ...
  const approveSuggestion = () => {
    if (selectedSuggestionId.length > 0) {
      dispatch(acceptWarningSignSuggestion(selectedSuggestionId));
      setActionType('');
      setSelectedSuggestionId('');
    }
  };
  ...
  const rejectSuggestion = () => {
    if (selectedSuggestionId.length > 0) {
      dispatch(rejectWarningSuggestion(selectedSuggestionId));
      setActionType('');
      setSelectedSuggestionId('');
    }
  };
        ...
        <SubmitConfirmation
          ...
          {actionType === 'accept' ? (
            <span className={classes.title}>
              This difficulty will become yours
              <br />
              when you approve.
            </span>
          ) : (
