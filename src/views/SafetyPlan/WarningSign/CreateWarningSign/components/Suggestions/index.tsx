import React, { useState } from 'react';
import { Suggestion } from 'types/safety';

import { makeStyles } from '@material-ui/styles';
import {
  IconButton,
  Checkbox,
  Divider,
  Table,
  TableBody,
  TableCell,
  TableRow
} from '@material-ui/core';
import CloseIcon from '@material-ui/icons/Close';

import { Button } from 'common/components';

const useStyles = makeStyles(() => ({
  header: {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    flexDirection: 'column',
    padding: '20px 0',
    position: 'relative'
  },
  title: {
    fontFamily: 'Scada',
    fontStyle: 'normal',
    fontWeight: 'bold',
    fontSize: '30px',
    lineHeight: '37px',
    color: '#73BA9B',
    marginTop: '5px'
  },
  body: {
    height: '340px',
    maxHeight: '340px',
    overflowY: 'auto',
    padding: '10px'
  },
  content: {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'flex-start',
    margin: '2px 0'
  },
  footer: {
    display: 'flex',
    alignItems: 'center',
    flexDirection: 'column',
    marginTop: '20px'
  },
  text: {
    fontFamily: 'Roboto',
    fontStyle: 'normal',
    fontWeight: 'normal',
    fontSize: '18px',
    lineHeight: '127.69%',
    color: '#37474F'
  },
  iconButton: {
    position: 'absolute',
    top: '4px',
    right: '-7px',
    padding: '0'
  },
  divider: {
    backgroundColor: '#73BA9B'
  },
  textButton: {
    cursor: 'pointer',
    fontFamily: 'Roboto',
    fontStyle: 'normal',
    fontWeight: 'bold',
    fontSize: '18px',
    lineHeight: '21px',
    color: '#003E1F',
    marginTop: '30px'
  }
}));

type Props = {
  close: () => void;
  add: (suggestion: Suggestion[]) => void;
};

const Suggestions: React.FC<Props> = ({ close, add }) => {
  const classes = useStyles();

  const [suggestions] = useState<Suggestion[]>([
    { id: '1', name: 'Having trouble sleeping or sleeping too much' },
    { id: '2', name: 'Struggling to keep up with my usual routine' },
    { id: '3', name: 'Drinking to cope' },
    { id: '4', name: 'Taking drugs' },
    { id: '5', name: 'Withdrawing from family and friends' },
    { id: '6', name: 'Eating a lot more, increase in appetite' },
    { id: '7', name: 'Eating a lot less, having no appetite  ' },
    { id: '8', name: 'Conflict with people' },
    { id: '9', name: 'Feeling hopeless' },
    { id: '10', name: 'Feeling like a burden' },
    { id: '11', name: 'Not looking after self' },
    { id: '12', name: 'Losing interest in things I normally enjoy' },
    { id: '13', name: 'Feeling down, depressed, unhappy' },
    { id: '14', name: 'Feeling confused' },
    { id: '15', name: 'Feeling nervous, anxious, worried' },
    { id: '16', name: 'Extreme high and low feelings' },
    { id: '17', name: 'Having distressing thoughts' }
  ]);

  const [selectedItems, setSelectedItems] = useState<string[]>([]);

  const handleSelectOne = (id: string) => {
    const selectedIndex = selectedItems.indexOf(id);
    let newSelectedItems: string[] = [];

    if (selectedIndex === -1) {
      newSelectedItems = newSelectedItems.concat(selectedItems, id);
    } else if (selectedIndex === 0) {
      newSelectedItems = newSelectedItems.concat(selectedItems.slice(1));
    } else if (selectedIndex === selectedItems.length - 1) {
      newSelectedItems = newSelectedItems.concat(selectedItems.slice(0, -1));
    } else if (selectedIndex > 0) {
      newSelectedItems = newSelectedItems.concat(
        selectedItems.slice(0, selectedIndex),
        selectedItems.slice(selectedIndex + 1)
      );
    }

    setSelectedItems(newSelectedItems);
  };

  const handleSubmit = () => {
    const finalSelectedItems: Suggestion[] = [];
    selectedItems.forEach(id => {
      const s = suggestions.find(n => n.id === id);
      finalSelectedItems.push(s!);
    });
    add(finalSelectedItems);
    close();
  };

  return (
    <div style={{ padding: '10px 20px' }}>
      <div className={classes.header}>
        <img
          src="data:image/png;base64,"
          alt=""
        />
        <span className={classes.title}>Warning Signs</span>
        <IconButton className={classes.iconButton} onClick={close}>
          <CloseIcon fontSize="large" style={{ fill: '#73BA9B' }} />
        </IconButton>
      </div>
      <Divider className={classes.divider} />
      <div className={classes.body}>
        <Table>
          <TableBody>
            {suggestions.map(suggestion => (
              <TableRow
                hover
                key={suggestion.id}
                selected={selectedItems.indexOf(suggestion.id) !== -1}
                onClick={() => handleSelectOne(suggestion.id)}>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedItems.indexOf(suggestion.id) !== -1}
                    color="primary"
                    value={selectedItems.indexOf(suggestion.id) !== -1}
                  />
                </TableCell>
                <TableCell>
                  <div>{suggestion.name}</div>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </div>
      <Divider className={classes.divider} />
      <div className={classes.footer}>
        <Button type="primary" click={handleSubmit}>
          Add selected suggestions
        </Button>
        <span className={classes.textButton} onClick={close}>
          Cancel
        </span>
      </div>
    </div>
  );
};

export default Suggestions;
