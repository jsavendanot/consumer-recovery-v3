...
import { CreateStayWellForm, Suggestions } from './components';
...
const CreateStayWell: React.FC = () => {
  const classes = useStyles();
  const { history } = useRouter();
  const dispatch = useDispatch();

  const staywell: Item[] = useSelector(
  ...
  /** Suggestions */
  const [values, setValues] = useState<Suggestion[]>([...staywell]);
  ...
  const save = () => {
    ...
    dispatch(createStaywell(updatedValues));
    history.push('/safety/staywell');
  };
  ...
  const navStepDialog = (
    <Dialog
      ...
      <DialogContent className={classes.navSteps}>
        <NavSteps step={1} close={handleClose2} />
      </DialogContent>
    </Dialog>
  );
  ...
  function openSaveConfirmHandler() {
    values.length !== staywell.length ? setOpenSaveConfirm(true) : save();
  }
