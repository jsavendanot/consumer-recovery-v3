...
import {
  acceptStayWellSuggestion,
  rejectStayWellSuggestion
} from 'slices/suggestion/safety/staywell/action';
...
export const SuggestedItem: React.FC<Props> = ({ item }) => {
  ...
  const approveSuggestion = () => {
    if (selectedSuggestionId.length > 0) {
      dispatch(acceptStayWellSuggestion(selectedSuggestionId));
      setActionType('');
      setSelectedSuggestionId('');
    }
  };
  ...
  const rejectSuggestion = () => {
    if (selectedSuggestionId.length > 0) {
      dispatch(rejectStayWellSuggestion(selectedSuggestionId));
      setActionType('');
      setSelectedSuggestionId('');
    }
  };
        ...
        <SubmitConfirmation
          ...
          {actionType === 'accept' ? (
            <span className={classes.title}>
              This stay well will become yours
              <br />
              when you approve.
            </span>
          ) : (
