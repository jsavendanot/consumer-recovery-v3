import React, { useState } from 'react';
import { Suggestion } from 'types/story';

import { makeStyles } from '@material-ui/styles';
import {
  IconButton,
  Checkbox,
  Divider,
  Table,
  TableBody,
  TableCell,
  TableRow,
  Theme
} from '@material-ui/core';
import { Close } from '@material-ui/icons';

import { Button } from 'common/components';

const useStyles = makeStyles((theme: Theme) => ({
  root: {
    [theme.breakpoints.up('xs')]: {
      padding: '0px'
    },
    [theme.breakpoints.up('sm')]: {
      padding: '20px'
    },
    [theme.breakpoints.up('md')]: {
      width: '400px',
      padding: '20px'
    },
    [theme.breakpoints.up('lg')]: {
      width: '400px',
      padding: '20px'
    }
  },
  header: {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    flexDirection: 'column',
    padding: '20px 0',
    position: 'relative'
  },
  title: {
    fontFamily: 'Scada',
    fontStyle: 'normal',
    fontWeight: 'bold',
    fontSize: '30px',
    lineHeight: '37px',
    color: '#73BA9B',
    marginTop: '5px'
  },
  body: {
    height: '340px',
    maxHeight: '340px',
    overflowY: 'auto',
    padding: '10px'
  },
  content: {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'flex-start',
    margin: '2px 0'
  },
  footer: {
    display: 'flex',
    alignItems: 'center',
    flexDirection: 'column',
    marginTop: '20px'
  },
  text: {
    fontFamily: 'Roboto',
    fontStyle: 'normal',
    fontWeight: 'normal',
    fontSize: '18px',
    lineHeight: '127.69%',
    color: '#37474F'
  },
  iconButton: {
    position: 'absolute',
    top: '4px',
    right: '-7px',
    padding: '0'
  },
  divider: {
    backgroundColor: '#73BA9B'
  },
  textButton: {
    cursor: 'pointer',
    fontFamily: 'Roboto',
    fontStyle: 'normal',
    fontWeight: 'bold',
    fontSize: '18px',
    lineHeight: '21px',
    color: '#003E1F',
    marginTop: '30px'
  }
}));

type Props = {
  close: () => void;
  add: (values: Suggestion[]) => void;
  startIndex: number;
};

const Suggestions: React.FC<Props> = ({ close, add, startIndex }) => {
  const classes = useStyles();

  const [suggestions] = useState<Suggestion[]>([
    { id: `${startIndex++}`, name: 'Good relationship with my GP' },
    { id: `${startIndex++}`, name: 'Secure accommodation' },
    { id: `${startIndex++}`, name: 'Support of a family member' },
    { id: `${startIndex++}`, name: 'Have a supportive partner' },
    { id: `${startIndex++}`, name: 'A great pet' },
    { id: `${startIndex++}`, name: 'Spiritual support (i.e. church)' },
    { id: `${startIndex++}`, name: 'Part of a self-help group' },
    { id: `${startIndex++}`, name: 'Own a car' },
    { id: `${startIndex++}`, name: 'Play a musical instrument' },
    { id: `${startIndex++}`, name: 'sing' },
    { id: `${startIndex++}`, name: 'Crossword puzzles' },
    { id: `${startIndex++}`, name: 'Cooking' },
    { id: `${startIndex++}`, name: 'Running' },
    { id: `${startIndex++}`, name: 'Learning another language' },
    { id: `${startIndex++}`, name: 'Volunteer work' },
    { id: `${startIndex++}`, name: 'Reliable' },
    { id: `${startIndex++}`, name: 'Caring' },
    { id: `${startIndex++}`, name: 'Hard working' },
    { id: `${startIndex++}`, name: 'Friendly' },
    { id: `${startIndex++}`, name: 'Willing to stand up for others' },
    { id: `${startIndex++}`, name: 'Sensitive' },
    { id: `${startIndex++}`, name: 'Good parent' }
  ]);

  const [selectedItems, setSelectedItems] = useState<string[]>([]);

  const handleSelectOne = (id: string) => {
    const selectedIndex = selectedItems.indexOf(id);
    let newSelectedItems: string[] = [];

    if (selectedIndex === -1) {
      newSelectedItems = newSelectedItems.concat(selectedItems, id);
    } else if (selectedIndex === 0) {
      newSelectedItems = newSelectedItems.concat(selectedItems.slice(1));
    } else if (selectedIndex === selectedItems.length - 1) {
      newSelectedItems = newSelectedItems.concat(selectedItems.slice(0, -1));
    } else if (selectedIndex > 0) {
      newSelectedItems = newSelectedItems.concat(
        selectedItems.slice(0, selectedIndex),
        selectedItems.slice(selectedIndex + 1)
      );
    }

    setSelectedItems(newSelectedItems);
  };

  const handleSubmit = () => {
    const finalSelectedItems: Suggestion[] = [];
    selectedItems.forEach(id => {
      const s = suggestions.find(n => n.id === id);
      if (s) finalSelectedItems.push(s!);
    });
    add(finalSelectedItems);
    close();
  };

  return (
    <div className={classes.root}>
      <div className={classes.header}>
        <img
          src="data:image/png;base64,"
          alt=""
        />
        <span className={classes.title}>My strenghts</span>
        <IconButton className={classes.iconButton} onClick={close}>
          <Close fontSize="large" style={{ fill: '#73BA9B' }} />
        </IconButton>
      </div>
      <Divider className={classes.divider} />
      <div className={classes.body}>
        <Table>
          <TableBody>
            {suggestions.map(suggestion => (
              <TableRow
                hover
                key={suggestion.id}
                selected={selectedItems.indexOf(suggestion.id) !== -1}
                onClick={event => handleSelectOne(suggestion.id)}>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedItems.indexOf(suggestion.id) !== -1}
                    color="primary"
                    value={selectedItems.indexOf(suggestion.id) !== -1}
                  />
                </TableCell>
                <TableCell>
                  <div>{suggestion.name}</div>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </div>
      <Divider className={classes.divider} />
      <div className={classes.footer}>
        <Button type="primary" click={handleSubmit}>
          Add selected suggestions
        </Button>
        <span className={classes.textButton} onClick={close}>
          Cancel
        </span>
      </div>
    </div>
  );
};

export default Suggestions;
